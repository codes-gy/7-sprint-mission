...
SELECT id, date, time FROM orders;
-- 2. `orders`테이블에서 `id` 가 `423`인 주문을 조회하세요.
SELECT id, date, time FROM orders WHERE id = 423;
-- 3. `orders` 테이블에서 총 주문 건수를 `total_orders`라는 이름으로 구하세요.
SELECT COUNT(*) AS total_orders FROM orders;
-- 4. `orders` 테이블에서 최신 순으로 주문을 조회하세요. (`date`, `time` 컬럼이 분리되어 있다는 점에 주의)
SELECT id, date, time FROM orders ORDER BY date DESC, time DESC;
-- 5. `orders` 테이블에서 오프셋 기반 페이지네이션된 목록을 조회합니다. 페이지 크기가 10이고 최신순일 때, 첫 번째 페이지를 조회하세요.
SELECT id, date, time FROM orders ORDER BY date DESC, time DESC LIMIT 10;
-- 6. `orders` 테이블에서 오프셋 기반 페이지네이션된 목록을 조회합니다. 페이지 크기가 10이고 최신순일 때 5번째 페이지를 조회하세요.
SELECT id, date, time FROM orders ORDER BY date DESC, time DESC LIMIT 10 OFFSET (5 - 1) * 10;
-- 7. `orders` 테이블에서 커서 페이지네이션된 목록을 조회합니다. 페이지 크기가 10이고 최신순일때, `id` 값을 기준으로 커서를 사용합시다. 커서의 값이 `42`일 때 다음 페이지를 조회하세요.
SELECT id, date, time FROM orders WHERE id < 42 ORDER BY id DESC LIMIT 10;
-- 8. `orders` 테이블에서 2025년 3월에 주문된 내역만 조회하세요.
SELECT id, date, time FROM orders WHERE date >= '2025-03-01' AND date <= '2025-03-31';
--------------------------------------------------
SELECT id, date, time FROM orders WHERE date BETWEEN '2025-03-01' AND '2025-03-31';
-- 9. `orders` 테이블에서 2025년 3월 12일 오전에 주문된 내역만 조회하세요.
SELECT id, date, time FROM orders WHERE date = '2025-03-12' AND time < '12:00:00';
...
SELECT
    pizza_id, COUNT(order_id) as order_count
FROM order_details
GROUP BY pizza_id;
-- 2. `order_details` 테이블에서 각 피자(`pizza_id`)별로 총 주문 수량을 구하세요.
SELECT
    pizza_id, SUM(quantity) as total_quantity
FROM order_details
GROUP BY pizza_id;
...
FROM orders
GROUP BY date
HAVING COUNT(*) >= 80 ORDER BY order_count DESC;
...
SELECT
    A.pizza_id, SUM(A.quantity * B.price) AS total_revenue
FROM order_details A
JOIN pizzas B ON A.pizza_id = B.id
GROUP BY A.pizza_id;
...
FROM orders A
JOIN order_details B ON B.order_id = A.id
GROUP BY A.date
ORDER BY A.date;
...
*/
SELECT
    A.id, A.type_id, A.size, A.price,
    SUM(B.quantity) AS total_quantity
FROM pizzas A
JOIN order_details B ON A.id = B.pizza_id
GROUP BY A.id
ORDER BY total_quantity DESC
LIMIT 10;
...
SELECT
    ...
    SUM(B.quantity * C.price) AS total_amount
FROM orders A
JOIN order_details B ON A.id = B.order_id
JOIN pizzas C ON B.pizza_id = C.id
WHERE A.date::TEXT LIKE '2025-03%'
GROUP BY A.date
ORDER BY A.date ASC;
--------------------------------------------------
SELECT
...
JOIN order_details B ON A.id = B.order_id
JOIN pizzas C ON B.pizza_id = C.id
WHERE A.date BETWEEN '2025-03-01' AND '2025-03-31'
GROUP BY A.date
ORDER BY A.date ASC;
...
*/
SELECT
    C.name AS pizza_name, B.size AS pizza_size, B.price AS pizza_price,
    A.quantity, (A.quantity * B.price) AS total_amount
FROM order_details A
JOIN pizzas B ON A.pizza_id = B.id
JOIN pizza_types C ON B.type_id = C.id
WHERE A.order_id = 78;
